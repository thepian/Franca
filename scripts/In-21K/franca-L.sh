#!/bin/bash
#SBATCH --job-name=franca_L_IN21k   # job name
#SBATCH -A fhx@h100
...
#SBATCH --output=slurm_jobs_logs/stdout/Imagenet_21k/francaL.out
#SBATCH --error=slurm_jobs_logs/stdout/Imagenet_21k/francaL.err
...
srun python franca/train/train.py \
  --config-file=franca/configs/train/IN21K/vitl14.yaml  \
  --output-dir= <path to output dir>/Imagenet21k/francaL/

# srun python dinov2/test_weights.py