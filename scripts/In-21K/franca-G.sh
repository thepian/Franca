#!/bin/bash
#SBATCH --job-name=franca_G_IN21k   # job name
#SBATCH -A fzh@h100
...
#SBATCH --output=slurm_jobs_logs/stdout/Imagenet_21k/francaG.out
#SBATCH --error=slurm_jobs_logs/stdout/Imagenet_21k/francaG.err
...
srun python franca/train/train.py \
  --config-file=franca/configs/train/IN21K/vitg14.yaml  \
  --output-dir= <path to output>/Imagenet21k/francaG/
