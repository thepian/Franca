#!/bin/bash
#SBATCH --job-name=franca_G_LAION   # job name
#SBATCH -A zvc@h100
...
#SBATCH --output=slurm_jobs_logs/stdout/LAION/francaG.out
#SBATCH --error=slurm_jobs_logs/stdout/LAION/francaG.err
...
srun python franca/train/train.py \
  --config-file=franca/configs/train/LAION/vitg14.yaml  \
  --output-dir= <path to output>/LAION/francaG/