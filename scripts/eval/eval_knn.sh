...
#SBATCH --gres=gpu:1                    # number of GPUs per task (max total 4 for H100 nodes)
...
#SBATCH --output=slurm_jobs_logs/stdout/eval/knn/FrancaG.out
#SBATCH --error=slurm_jobs_logs/stdout/eval/knn/FrancaG.err
#SBATCH --qos=qos_gpu_h100-dev

cd $WORK/Franca
mkdir -p slurm_jobs_logs/stdout/eval/knn
...
srun python franca/eval/knn.py \
    --config-file <path to output dir>/config.yaml \
    --pretrained-weights <path to output dir>/eval/training_624999/teacher_checkpoint.pth \
    --output-dir <path to output dir>/eval/training_624999/knn \
